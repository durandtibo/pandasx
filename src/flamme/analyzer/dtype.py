...
class ColumnDtypeAnalyzer(BaseAnalyzer):
    r"""Implements a column data type analyzer.
    ...
    .. code-block:: pycon
        ...
        >>> import numpy as np
        >>> import pandas as pd
        >>> from flamme.analyzer import ColumnDtypeAnalyzer
        >>> analyzer = ColumnDtypeAnalyzer()
        >>> analyzer
        ColumnDtypeAnalyzer()
        >>> df = pd.DataFrame(
        ...     {
    ...
    def analyze(self, df: DataFrame) -> ColumnDtypeSection:
        return ColumnDtypeSection(df.dtypes)
