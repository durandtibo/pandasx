...
To make the package as slim as possible, only the packages required to use `flamme` are
installed.
...
To install `flamme` from source, you can follow the steps below. First, you will need to
...
```shell
git clone git@github.com:durandtibo/flamme.git
```
...
```shell
conda activate flamme
```

This example uses `conda` to create a virtual environment, but you can use other tools or
configurations. Then, you should install the required package to use `flamme` with the
following command:
