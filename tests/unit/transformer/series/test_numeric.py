...
def test_to_numeric_series_transformer_transform() -> None:
    ...
    out = transformer.transform(pd.Series(["1", "2", "3", "4", 5]))
    assert_series_equal(out, pd.Series([1, 2, 3, 4, 5]))


def test_to_numeric_series_transformer_transform_kwargs() -> None:
    transformer = ToNumeric(errors="coerce")
    out = transformer.transform(pd.Series(["1", "2", "3", "4", "a5"]))
    assert_series_equal(out, pd.Series([1, 2, 3, 4, float("nan")]))
